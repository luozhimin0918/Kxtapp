...
public class RefreshableView extends LinearLayout implements OnTouchListener {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		super.onLayout(changed, l, t, r, b); // LinearLayout的方法
		if (changed && !loadOnce) {
	 ...
	 */
	public void finishRefreshing(int a) {
		...
		case 41:
			...
			break;
		case 60:
			headerLayoutParams.topMargin = -90;
			header.setLayoutParams(headerLayoutParams);
			currentStatus = STATUS_REFRESH_FINISHED;
			break;
		default:
	...
	private void rotateArrow() {
		float pivotX = arrow.getWidth() / 2f; // pivot:支点；枢纽；中心点
		...
		if (currentStatus == STATUS_PULL_TO_REFRESH) {
			fromDegrees = 180f;
			toDegrees = 360f;
		} else if (currentStatus == STATUS_RELEASE_TO_REFRESH) { // release:释放
			fromDegrees = 0f;
			toDegrees = 180f;
		}
		...
		animation.setFillAfter(true); // 播放完动画是否回到原位
		arrow.startAnimation(animation); // 这是View的方法
	}
