...
public class Utils {
	 ...
	 * @return
	 */
	public  static CjInfo  paseJson(String json){
		CjInfo info=new CjInfo();
		...
		info.setPredictTime(str1s[0]);
		info.setNfi(str1s[1]);
		info.setBefore(str1s[2]);
		info.setForecast(str1s[3]);
		info.setReality(str1s[4]);
		info.setNature(str1s[5]);
		
		info.setEffect(str2);
		
		info.setDate(str3s[0]);
		info.setState(str3s[1]);
